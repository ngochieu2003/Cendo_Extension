...
async function refreshNext() {
  const st = await send({ type: "TENNIS_STATUS" });
  const el = $("#next");
...
$("#runNow").onclick = async () => {
  ...
  const r = await send({ type: "TENNIS_RUN_NOW" });
...
chrome.runtime.onMessage.addListener((msg) => {
  if (!msg) return;
  if (msg.type === "TENNIS_PROGRESS_UI") setStatus("Đang chạy: " + msg.text, "");
  if (msg.type === "TENNIS_DONE") {
    const l = msg.log;
