/* =========================================================================
   Móc khoá Clicky Tennis 3D – LUỒNG TỰ ĐỘNG (chạy trong tab cendo.work)
   - Nhận lệnh từ bg.js (alarm hằng ngày / nút "Chạy ngay").
   - Quét đơn có tag "Up đủ thông tin" qua /api/orders/search, lấy items qua
     /api/orders/{id}/items, chỉ xử lý item Tennis mà CẢ mockup và
     design đều trống. Đơn khác tuyệt đối không đụng.
   - Render bằng window.__TENNIS_API__ (content.js), upload qua
     POST /api/orders/{orderId}/items/{itemId}/upload-media (field=mockup|design).
   ========================================================================= */
(() => {
  "use strict";
  if (window.__TENNIS_AUTO_LOADED__) return;
  window.__TENNIS_AUTO_LOADED__ = true;
  ...
  /* --------------------------- Nhận diện item --------------------------- */
  // Item Tennis: theo product_name "Móc khoá Clicky Tennis 3D" (đơn thật 6a7aef55…,
  // sku_id dạng NDT260615-3-8 không đặc trưng, fulfillment_sku trống) — sửa nếu shop đổi tên
  const itemText = (it) => `${it.product_name || ""} ${it.variant_name || ""} ${it.sku_id || ""} ${it.fulfillment_sku || ""}`;
  const isTennis = (it, H) => H.isTennisText(itemText(it));
  // Màu: variant_name kiểu "Màu: Màu Xanh Bạc Hà, Phân loại: In tên" (7 màu có mẫu:
  // Đen/Đỏ/Vàng/Xanh Bạc Hà/Xanh Lá/Hồng/Cam). Màu có tên nhưng chưa có mẫu
  // (Xanh Dương, Tím…) -> "no-tpl:<tên>" để báo lý do; không đọc được -> null.
  ...
  async function processOrder(order, H) {
    ...
    const cl = items.filter((it) => isTennis(it, H));
    if (!cl.length) return null; // không phải đơn Tennis -> bỏ qua im lặng
    ...
    // Tên: dòng thứ n của External note -> item Tennis thứ n (theo thứ tự trong đơn)
    ...
    if (names.length !== cl.length) {
      return { ...base, status: "skip", why: `lệch số lượng: note có ${names.length} dòng tên nhưng đơn có ${cl.length} item Tennis — làm tay (note: "${noteRaw.replace(/\n/g, " | ").slice(0, 80)}")` };
    }
    ...
    for (let i = 0; i < cl.length; i++) {
      ...
      if (!H.hasTpl(color)) return { ...base, status: "skip", why: `item ${i + 1} màu ${String(color).replace(/^no-tpl:/, "")} chưa có mẫu mockup Tennis (${it.variant_name || ""}) — làm tay` };
    ...
    for (const j of jobs) {
      ...
      const mockF = await H.canvasToFile(mockC, `mockup-tennis-${tag}.png`);
      const desF = await H.canvasToFile(desC, `design-tennis-${tag}.png`);
  ...
  async function runFlow(cfg, progress) {
    ...
    running = true;
    const H = window.__TENNIS_API__;
    const started = Date.now();
    const rep = { pagesScanned: 0, ordersScanned: 0, cl: 0, done: [], skipped: [], errors: [], already: 0, fatal: null, scope: cfg.pages };
    try {
      if (!H) throw new Error("content.js chưa nạp (window.__TENNIS_API__ trống). Reload trang cendo.work.");
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (!msg) return;
    if (msg.type === "TENNIS_PROGRESS_UI") { setWidgetStatus("⏳ " + msg.text); return; }
    if (msg.type === "TENNIS_DONE" && msg.log) { const l = msg.log; setWidgetStatus(`Xong: ✅ ${l.done} · ⚠️ ${l.skipped} · ❌ ${l.errors}${l.fatal ? " · DỪNG: " + l.fatal : ""} · Lark: ${l.lark}`, l.errors || l.fatal ? "err" : "ok"); refreshWidget(); return; }
    if (msg.type !== "TENNIS_RUN") return;
    if (msg.ping) { sendResponse({ ok: true, running }); return; }
    if (running) { sendResponse({ ok: false, error: "Luồng đang chạy" }); return; }
    sendResponse({ ok: true, started: true });
    toast("🎾 Tennis: bắt đầu chạy tự động…", "info");
    const runId = msg.runId || String(Date.now());
    const send = (m) => { try { chrome.runtime.sendMessage({ ...m, runId }); } catch (e) {} };
    runFlow(msg.cfg || { pages: 1 }, (p) => { toast("🎾 " + p, "info"); setWidgetStatus("⏳ " + p); send({ type: "TENNIS_PROGRESS", text: p }); })
      .then((rep) => {
        toast(`🎾 Xong: làm ${rep.done.length} · bỏ qua ${rep.skipped.length} · lỗi ${rep.errors.length}`, rep.errors.length || rep.fatal ? "err" : "ok");
        send({ type: "TENNIS_REPORT", report: rep, trigger: msg.trigger || "manual" });
      })
      .catch((e) => send({ type: "TENNIS_REPORT", report: { fatal: e.message, done: [], skipped: [], errors: [], already: 0, pagesScanned: 0, ordersScanned: 0, cl: 0, scope: (msg.cfg || {}).pages }, trigger: msg.trigger || "manual" }));
  });
  ...
  function buildWidget() {
    if (document.getElementById("tennis-auto-fab")) return;
    const fab = document.createElement("button"); fab.id = "tennis-auto-fab"; fab.type = "button";
    fab.innerHTML = "🎾 Auto Tennis: …";
    const panel = document.createElement("div"); panel.id = "tennis-auto-panel"; panel.style.display = "none";
    panel.innerHTML = `
      <div class="tennis-ap-head"><b>🎾 Tự động làm đơn Móc khoá Clicky Tennis</b><button type="button" class="tennis-ap-close" title="Đóng">×</button></div>
      <label class="tennis-ap-switch"><input type="checkbox" id="tennis-ap-enabled"><span>Bật chạy tự động hằng ngày</span></label>
      <div class="tennis-ap-row"><span>Giờ chạy</span><input type="time" id="tennis-ap-time" value="08:00"></div>
      <div class="tennis-ap-row"><span>Quy mô</span><select id="tennis-ap-pages">${PAGE_OPTS.map(([v, l]) => `<option value="${v}">${l}</option>`).join("")}</select></div>
      <div class="tennis-ap-hint" id="tennis-ap-next"></div>
      <div class="tennis-ap-actions">
        <button type="button" class="tennis-ap-save" id="tennis-ap-save">Lưu</button>
        <button type="button" class="tennis-ap-run" id="tennis-ap-run">▶ Chạy ngay</button>
        <button type="button" class="tennis-ap-more" id="tennis-ap-more" title="Webhook Lark, lịch sử chạy…">Cài đặt đầy đủ</button>
      </div>
      <div class="tennis-ap-status info" id="tennis-ap-status">Chỉ làm đơn "Up đủ thông tin" có Mockup & Design trống.</div>`;
    document.body.appendChild(fab); document.body.appendChild(panel);
    const $ = (s) => panel.querySelector(s);
    W = { fab, panel, enabled: $("#tennis-ap-enabled"), time: $("#tennis-ap-time"), pages: $("#tennis-ap-pages"), next: $("#tennis-ap-next"), status: $("#tennis-ap-status"), run: $("#tennis-ap-run") };
    fab.onclick = () => { const open = panel.style.display !== "none"; panel.style.display = open ? "none" : "block"; if (!open) refreshWidget(); };
    $(".tennis-ap-close").onclick = () => { panel.style.display = "none"; };
    const save = async () => {
  ...
  async function refreshWidget() {
    ...
    W.fab.innerHTML = `🎾 Auto Tennis: <b>${cfg.enabled ? "BẬT " + pad2(cfg.hour) + ":" + pad2(cfg.minute) : "TẮT"}</b> · ${scopeLabel(cfg.pages)}`;
    W.fab.classList.toggle("on", !!cfg.enabled);
    const st = await bgSend({ type: "TENNIS_STATUS" });
    if (st) {
  ...
  function setWidgetStatus(t, cls) { if (W) { W.status.textContent = t; W.status.className = "tennis-ap-status " + (cls || "info"); } }
  chrome.storage.onChanged.addListener((ch, area) => { if (area === "local") refreshWidget(); });
  const ensureWidget = () => { if (!document.getElementById("tennis-auto-fab")) { W = null; buildWidget(); } };
