/* =========================================================================
   ...
   - Màu (mẫu mockup): mục "Màu" trong variant của từng item
     (Đen · Đỏ · Vàng · Xanh Bạc Hà · Xanh Lá · Hồng · Cam) — 7 mẫu.
   - Mockup 1500×1500: ảnh mẫu theo màu + tên khách IN HOA, font Lemonada Bold,
     ...
     Cầu lông / Chú Cừu nhưng XOAY DỌC 90° (đọc từ trên xuống, đầu chữ quay
     sang phải, vòng tròn ở trên cùng) đặt bên phải móc khoá.
   - Design 1254×1254: nền trắng, chữ đen Arial Bold, canh giữa (giống TKB).
   ========================================================================= */
(() => {
  "use strict";
  if (window.__TENNIS_LOADED__) return;
  window.__TENNIS_LOADED__ = true;
  ...
  /* ======================= Thông số 7 mẫu màu =======================
     ...
     cách khớp 7 ảnh mẫu ĐÃ điền THUYTHU (thư mục "VÍ dụ") với bản chưa điền
     (match ≥0.99). Cả 7 mẫu đặt sticker ở CÙNG một chỗ:
       size  = cỡ chữ Lemonada Bold mặc định (px)
       baseX = toạ độ X của đường chân chữ (chữ xoay dọc nên baseline là đường
               THẲNG ĐỨNG; đầu chữ quay sang phải, tức x > baseX)
     ...
     ================================================================== */
  /* Style sticker (giống hệt Cầu lông/Chú Cừu, tỷ lệ theo cỡ chữ `size`):
     - chữ TRẮNG, nền blob ĐỎ ôm theo chữ (stroke đỏ dày, bo tròn),
     - vòng tròn đỏ trang trí nằm TRƯỚC chữ (bên trái trong hệ chữ = phía trên
       sau khi xoay dọc),
  ...
  const STICKER = {
    ...
    // ring: tâm cách mép đầu chữ `gap`×size, cao hơn baseline `dy`×size,
    //       bán kính giữa `midR`×size, dày nét `width`×size
    rotate: Math.PI / 2, // xoay 90° thuận chiều kim đồng hồ (đọc từ trên xuống)
  };
  const POS = { baseX: 1060, cy: 938, size: 119, maxL: 920 }; // chung cho 7 mẫu
  const TPL = {
    black:  { key: "black",  label: "Đen",          img: "assets/black.jpg",  ...POS },
    red:    { key: "red",    label: "Đỏ",           img: "assets/red.jpg",    ...POS },
    yellow: { key: "yellow", label: "Vàng",         img: "assets/yellow.jpg", ...POS },
    mint:   { key: "mint",   label: "Xanh Bạc Hà",  img: "assets/mint.jpg",   ...POS },
    green:  { key: "green",  label: "Xanh Lá",      img: "assets/green.jpg",  ...POS },
    pink:   { key: "pink",   label: "Hồng",         img: "assets/pink.jpg",   ...POS },
    orange: { key: "orange", label: "Cam",          img: "assets/orange.jpg", ...POS },
  };
  ...
  function tplCoords(t, w, h) {
    return {
      size: t.size != null ? t.size : Math.round(h * 0.08),
      baseX: t.baseX != null ? t.baseX : Math.round(w * 0.706),
      cy: t.cy != null ? t.cy : Math.round(h * 0.625),
      maxL: t.maxL != null ? t.maxL : Math.round(h * 0.61),
    };
  }
  ...
  /* ------------------------------ Font ------------------------------ */
  const FONT_LEMONADA = "TENNIS-Lemonada";
  let fontsReady = null;
  function loadFonts() {
  ...
  // (đọc từ trên xuống), đầu chữ (phía trên baseline) -> quay sang PHẢI.
  ...
  // Variant thật (đơn 6a7aef55…): "Màu: Màu Xanh Bạc Hà, Phân loại: In tên"
  ...
  function colorFromText(s) {
    ...
    if (/xanh\s*(duong|bien|nuoc)/.test(t)) return "no-tpl:Xanh Dương";
    ...
    if (/\btim\b/.test(t)) return "no-tpl:Tím";
    if (/\btrang\b/.test(t)) return "no-tpl:Trắng";
    return null;
  }
  // Fulfillment SKU (nếu shop có đặt): …-{Black|Red|Yellow|Mint|Green|Pink|Orange}-…
  ...
  // Item Tennis: product_name "Móc khoá Clicky Tennis 3D" (Thuan xác nhận 27/08/2026)
  const isTennisText = (s) => /tennis/.test(noAccent(s));

  // Quét các item Tennis trên trang đơn (theo đúng thứ tự hiển thị).
  ...
  function buildModal() {
    ...
    overlay.id = "tennis-overlay";
    overlay.innerHTML = `
      <div id="tennis-modal">
        <div class="tennis-head"><h2>🎾 Tạo Mockup &amp; Design – Móc khoá Clicky Tennis 3D</h2><button class="tennis-close" title="Đóng">×</button></div>
        <div class="tennis-body">
          <div class="tennis-left">
            <div id="tennis-warn" class="tennis-warn" style="display:none"></div>
            <div class="tennis-field"><label>Các item trong đơn (tên lấy theo thứ tự dòng của External note)</label>
              <div id="tennis-items" class="tennis-items"></div>
            </div>
            <div class="tennis-field"><label>Chỉnh chữ cho item đang chọn</label>
              <div class="tennis-rf-row"><span>Cỡ %</span><input type="number" id="tennis-size" class="tennis-num tennis-num-sm" min="40" max="200" step="5" value="100">
                <span>X</span><input type="number" id="tennis-tx" class="tennis-num tennis-num-sm" step="1" value="0">
                <span>Y</span><input type="number" id="tennis-ty" class="tennis-num tennis-num-sm" step="1" value="0">
                <button type="button" id="tennis-treset">Đặt lại</button></div>
              <div class="tennis-hint">Kéo thẳng trên ảnh preview để dời chữ · lăn chuột để đổi cỡ. Chữ xoay dọc bên phải móc khoá, luôn IN HOA, tên quá dài tự thu nhỏ.</div>
            </div>
          </div>
          <div class="tennis-right"><div class="tennis-previews">
            <div class="tennis-pv"><div class="tennis-pv-title" id="tennis-pv-label">Mockup</div>
              <canvas id="tennis-pv-mock" class="tennis-draggable"></canvas></div>
            <div class="tennis-pv"><div class="tennis-pv-title">Design (1254×1254) · nền trắng, chữ đen Arial Bold</div>
              <canvas id="tennis-pv-design"></canvas></div>
          </div></div>
          <div id="tennis-status" class="info"></div>
          <div class="tennis-actions">
            <button class="tennis-btn-dl" id="tennis-dl">⬇ Tải PNG (tất cả)</button>
            <button class="tennis-btn-fill" id="tennis-fill">✅ Điền vào Mockup &amp; Design</button>
          </div>
        </div>
      </div>`;
    ...
    els = {
      overlay, warn: $("#tennis-warn"), list: $("#tennis-items"), size: $("#tennis-size"),
      tx: $("#tennis-tx"), ty: $("#tennis-ty"), tReset: $("#tennis-treset"), pvLabel: $("#tennis-pv-label"),
      pvMock: $("#tennis-pv-mock"), pvDesign: $("#tennis-pv-design"), status: $("#tennis-status"),
      btnDl: $("#tennis-dl"), btnFill: $("#tennis-fill"),
    };
    ...
    $(".tennis-close").onclick = closeModal;
    overlay.addEventListener("click", (e) => { if (e.target === overlay) closeModal(); });
    els.btnDl.onclick = doDownload; els.btnFill.onclick = doFill;

    let drag = null;
    els.pvMock.addEventListener("pointerdown", (e) => { drag = { x: e.clientX, y: e.clientY }; els.pvMock.setPointerCapture(e.pointerId); els.pvMock.classList.add("tennis-grabbing"); });
    ...
    const end = () => { if (drag) { drag = null; els.pvMock.classList.remove("tennis-grabbing"); } };
  ...
  function renderItemList() {
    ...
    state.items.forEach((it, i) => {
      ...
      card.className = "tennis-item" + (i === state.active ? " sel" : "") + (it.name ? "" : " empty");
      const tk = it.detected ? "Màu: " + TPL[it.color].label : (it.colorNote ? `không có mẫu màu ${it.colorNote}` : "không đọc được Màu");
      card.innerHTML = `
        <div class="tennis-item-head">
          <b>Item ${i + 1}</b>
          <span class="tennis-item-tk ${it.detected ? "ok" : "err"}">${tk}</span>
          <span class="tennis-item-sku">${it.sku || ""}</span>
        </div>
        <div class="tennis-item-row">
          <input class="tennis-item-name" type="text" placeholder="Tên khách…" value="">
          <select class="tennis-item-color">${TPL_ORDER.map((k) => `<option value="${k}">${TPL[k].label}</option>`).join("")}</select>
        </div>`;
      const input = card.querySelector(".tennis-item-name");
      input.value = it.name || "";
      ...
      const sel = card.querySelector(".tennis-item-color");
      sel.value = it.color;
    ...
    if (!state.items.length) els.list.innerHTML = '<div class="tennis-hint">Không thấy item Móc khoá Clicky Tennis nào trên trang này.</div>';
  }
  function select(i) {
    if (i === state.active) { syncTools(); return; }
    state.active = i;
    els.list.querySelectorAll(".tennis-item").forEach((c, k) => c.classList.toggle("sel", k === i));
    syncTools(); schedule();
  }
  function syncTools() {
  ...
  function refreshWarn() {
    ...
    const msgs = [];
    if (n && m && m !== n) msgs.push(`⚠️ External note có ${m} dòng tên nhưng đơn có ${n} item Tennis — kiểm tra lại rồi điền tay cho khớp.`);
  ...
  function toast(msg, cls) {
    let t = document.getElementById("tennis-toast");
    if (!t) { t = document.createElement("div"); t.id = "tennis-toast"; document.body.appendChild(t); }
  ...
  async function buildFiles(it, i) {
    ...
    const mock = await canvasToFile(await renderMockup(document.createElement("canvas"), it.color, it.name, { dx: it.dx, dy: it.dy }, it.sizePct / 100), `mockup-tennis-${tag}.png`);
    const des = await canvasToFile(await renderDesign(document.createElement("canvas"), it.name), `design-tennis-${tag}.png`);
    return { mock, des };
  }
  ...
  function waitFillResult(timeout) {
    return new Promise((resolve) => {
      ...
      const on = (ev) => {
        ...
        if (ev.data.__tennis === "progress") { // còn chạy -> gia hạn chờ
        ...
        if (ev.data.__tennis === "result") done(ev.data);
      };
  ...
  async function doFill() {
    try {
      ...
      window.postMessage({ __tennis: "fill", items: payload }, "*");
  ...
  function ensureButton() {
    ...
    let btn = document.getElementById("tennis-open-btn");
    if (!onOrder) { if (btn) btn.remove(); return; }
    if (btn) return;
    btn = document.createElement("button"); btn.id = "tennis-open-btn"; btn.type = "button";
    btn.innerHTML = "🎾 Tạo Mockup Tennis";
  ...
  window.__TENNIS_API__ = {
    loadFonts, renderMockup, renderDesign, canvasToFile, loadBitmap,
    noteNames, cleanName, colorFromText, colorFromSku, hasTpl, isTennisText, noAccent,
    TPL, TPL_ORDER, DESIGN, STICKER, POS,
  };
})();
