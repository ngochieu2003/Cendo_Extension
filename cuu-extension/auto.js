/* =========================================================================
   Móc Khoá Clicky Chú Cừu Dễ Thương 3D – LUỒNG TỰ ĐỘNG (chạy trong tab cendo.work)
   - Nhận lệnh từ bg.js (alarm hằng ngày / nút "Chạy ngay").
   - Quét đơn có tag "Up đủ thông tin" qua /api/orders/search, lấy items qua
     /api/orders/{id}/items, chỉ xử lý item Chú Cừu mà CẢ mockup và
     design đều trống. Đơn khác tuyệt đối không đụng.
   - Render bằng window.__CUU_API__ (content.js), upload qua
     POST /api/orders/{orderId}/items/{itemId}/upload-media (field=mockup|design).
   ========================================================================= */
(() => {
  "use strict";
  if (window.__CUU_AUTO_LOADED__) return;
  window.__CUU_AUTO_LOADED__ = true;
  ...
  /* --------------------------- Nhận diện item --------------------------- */
  // Item Chú Cừu: theo tên sản phẩm "Móc Khoá Clicky Chú Cừu Dễ Thương 3D" (sửa lại nếu shop đổi tên)
  const itemText = (it) => `${it.product_name || ""} ${it.variant_name || ""} ${it.sku_id || ""} ${it.fulfillment_sku || ""}`;
  const isCuu = (it, H) => H.isCuuText(itemText(it));
  ...
  async function processOrder(order, H) {
    ...
    const cl = items.filter((it) => isCuu(it, H));
    if (!cl.length) return null; // không phải đơn Chú Cừu -> bỏ qua im lặng
    ...
    // Tên: dòng thứ n của External note -> item Chú Cừu thứ n (theo thứ tự trong đơn)
    ...
    if (names.length !== cl.length) {
      return { ...base, status: "skip", why: `lệch số lượng: note có ${names.length} dòng tên nhưng đơn có ${cl.length} item Chú Cừu — làm tay (note: "${noteRaw.replace(/\n/g, " | ").slice(0, 80)}")` };
    }
    ...
    // Đủ điều kiện: chỉ có 1 mẫu nên không cần đọc màu
    const jobs = [];
    for (let i = 0; i < cl.length; i++) {
      const it = cl[i];
      if (!empty.includes(it)) continue; // item này đã có đủ 2 ảnh -> giữ nguyên
      jobs.push({ it, i, color: "cuu", name: names[i] });
    }
    ...
    for (const j of jobs) {
      ...
      const mockF = await H.canvasToFile(mockC, `mockup-cuu-${tag}.png`);
      const desF = await H.canvasToFile(desC, `design-cuu-${tag}.png`);
      ...
      doneItems.push({ i: j.i + 1, name: j.name, design: "Chú Cừu", sku: j.it.sku_id, qty: j.it.quantity });
      await sleep(300);
    }
  ...
  async function runFlow(cfg, progress) {
    ...
    running = true;
    const H = window.__CUU_API__;
    const started = Date.now();
    const rep = { pagesScanned: 0, ordersScanned: 0, cl: 0, done: [], skipped: [], errors: [], already: 0, fatal: null, scope: cfg.pages };
    try {
      if (!H) throw new Error("content.js chưa nạp (window.__CUU_API__ trống). Reload trang cendo.work.");
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (!msg) return;
    if (msg.type === "CUU_PROGRESS_UI") { setWidgetStatus("⏳ " + msg.text); return; }
    if (msg.type === "CUU_DONE" && msg.log) { const l = msg.log; setWidgetStatus(`Xong: ✅ ${l.done} · ⚠️ ${l.skipped} · ❌ ${l.errors}${l.fatal ? " · DỪNG: " + l.fatal : ""} · Lark: ${l.lark}`, l.errors || l.fatal ? "err" : "ok"); refreshWidget(); return; }
    if (msg.type !== "CUU_RUN") return;
    if (msg.ping) { sendResponse({ ok: true, running }); return; }
    if (running) { sendResponse({ ok: false, error: "Luồng đang chạy" }); return; }
    sendResponse({ ok: true, started: true });
    toast("🐑 Chú Cừu: bắt đầu chạy tự động…", "info");
    const runId = msg.runId || String(Date.now());
    const send = (m) => { try { chrome.runtime.sendMessage({ ...m, runId }); } catch (e) {} };
    runFlow(msg.cfg || { pages: 1 }, (p) => { toast("🐑 " + p, "info"); setWidgetStatus("⏳ " + p); send({ type: "CUU_PROGRESS", text: p }); })
      .then((rep) => {
        toast(`🐑 Xong: làm ${rep.done.length} · bỏ qua ${rep.skipped.length} · lỗi ${rep.errors.length}`, rep.errors.length || rep.fatal ? "err" : "ok");
        send({ type: "CUU_REPORT", report: rep, trigger: msg.trigger || "manual" });
      })
      .catch((e) => send({ type: "CUU_REPORT", report: { fatal: e.message, done: [], skipped: [], errors: [], already: 0, pagesScanned: 0, ordersScanned: 0, cl: 0, scope: (msg.cfg || {}).pages }, trigger: msg.trigger || "manual" }));
  });
  ...
  function buildWidget() {
    if (document.getElementById("cuu-auto-fab")) return;
    const fab = document.createElement("button"); fab.id = "cuu-auto-fab"; fab.type = "button";
    fab.innerHTML = "🐑 Auto Chú Cừu: …";
    const panel = document.createElement("div"); panel.id = "cuu-auto-panel"; panel.style.display = "none";
    panel.innerHTML = `
      <div class="cuu-ap-head"><b>🐑 Tự động làm đơn Móc Khoá Chú Cừu</b><button type="button" class="cuu-ap-close" title="Đóng">×</button></div>
      <label class="cuu-ap-switch"><input type="checkbox" id="cuu-ap-enabled"><span>Bật chạy tự động hằng ngày</span></label>
      <div class="cuu-ap-row"><span>Giờ chạy</span><input type="time" id="cuu-ap-time" value="08:00"></div>
      <div class="cuu-ap-row"><span>Quy mô</span><select id="cuu-ap-pages">${PAGE_OPTS.map(([v, l]) => `<option value="${v}">${l}</option>`).join("")}</select></div>
      <div class="cuu-ap-hint" id="cuu-ap-next"></div>
      <div class="cuu-ap-actions">
        <button type="button" class="cuu-ap-save" id="cuu-ap-save">Lưu</button>
        <button type="button" class="cuu-ap-run" id="cuu-ap-run">▶ Chạy ngay</button>
        <button type="button" class="cuu-ap-more" id="cuu-ap-more" title="Webhook Lark, lịch sử chạy…">Cài đặt đầy đủ</button>
      </div>
      <div class="cuu-ap-status info" id="cuu-ap-status">Chỉ làm đơn "Up đủ thông tin" có Mockup & Design trống.</div>`;
    document.body.appendChild(fab); document.body.appendChild(panel);
    const $ = (s) => panel.querySelector(s);
    W = { fab, panel, enabled: $("#cuu-ap-enabled"), time: $("#cuu-ap-time"), pages: $("#cuu-ap-pages"), next: $("#cuu-ap-next"), status: $("#cuu-ap-status"), run: $("#cuu-ap-run") };
    fab.onclick = () => { const open = panel.style.display !== "none"; panel.style.display = open ? "none" : "block"; if (!open) refreshWidget(); };
    $(".cuu-ap-close").onclick = () => { panel.style.display = "none"; };
    const save = async () => {
  ...
  async function refreshWidget() {
    ...
    W.fab.innerHTML = `🐑 Auto Chú Cừu: <b>${cfg.enabled ? "BẬT " + pad2(cfg.hour) + ":" + pad2(cfg.minute) : "TẮT"}</b> · ${scopeLabel(cfg.pages)}`;
    W.fab.classList.toggle("on", !!cfg.enabled);
    const st = await bgSend({ type: "CUU_STATUS" });
    if (st) {
  ...
  function setWidgetStatus(t, cls) { if (W) { W.status.textContent = t; W.status.className = "cuu-ap-status " + (cls || "info"); } }
  chrome.storage.onChanged.addListener((ch, area) => { if (area === "local") refreshWidget(); });
  const ensureWidget = () => { if (!document.getElementById("cuu-auto-fab")) { W = null; buildWidget(); } };
