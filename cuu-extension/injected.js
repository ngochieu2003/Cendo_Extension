/* =========================================================================
   Móc Khoá Clicky Chú Cừu 3D – script chạy ở MAIN world của cendo.work.
...
(function () {
  "use strict";
  if (window.__CUU_MAIN__) return;
  window.__CUU_MAIN__ = true;
  ...
  function injectFileToInput(input, file) {
    try {
      ...
      console.warn("[CUU] injectFileToInput lỗi:", e);
      return false;
    }
  }
  ...
  window.addEventListener("message", async (ev) => {
    if (ev.source !== window || !ev.data || ev.data.__cuu !== "fill") return;
    const list = Array.isArray(ev.data.items) ? ev.data.items : [ev.data];
    const results = [];
    const tick = (label, step) => window.postMessage({ __cuu: "progress", done: results.length, total: list.length, label, step }, "*");
    ...
    window.postMessage({ __cuu: "result", results }, "*");
  });

  /* --------- Chẩn đoán: gõ __CUU_DIAG__() trong Console để soi --------- */
  window.__CUU_DIAG__ = () => {
    const rows = [...document.querySelectorAll('[class*="media_row"]')];
    return {
      rows: rows.length,
