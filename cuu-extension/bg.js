/* =========================================================================
   Móc Khoá Clicky Chú Cừu 3D – Service worker: đặt lịch (chrome.alarms), mở tab
   cendo.work, ra lệnh chạy cho auto.js, nhận báo cáo và gửi Lark.
   ========================================================================= */
const ALARM = "cuu-daily";
const DEFAULTS = {
...
async function ping(tabId) {
  try { const r = await chrome.tabs.sendMessage(tabId, { type: "CUU_RUN", ping: true }); return !!(r && r.ok); } catch (e) { return false; }
}
...
async function startRun(trigger) {
  ...
  try {
    ...
    const r = await chrome.tabs.sendMessage(tab.id, { type: "CUU_RUN", runId, trigger, cfg: { pages: Number(cfg.pages) || 0 } });
...
function buildLarkText(rep, trigger, cfg) {
  ...
  const L = [];
  L.push(`${cfg.larkKeyword} 🐑 Móc Khoá Clicky Chú Cừu 3D – tự động làm Mockup/Design`);
  L.push(`⏰ ${fmtTime()} · ${trigger === "schedule" ? "chạy theo lịch" : "chạy tay"} · phạm vi: ${scope}`);
  if (rep.fatal) L.push(`❌ LỖI DỪNG LUỒNG: ${rep.fatal}`);
  L.push(`📄 Đã quét ${rep.pagesScanned} trang / ${rep.ordersScanned} đơn "Up đủ thông tin" · đơn Chú Cừu: ${rep.cl} (đã làm từ trước: ${rep.already})`);
...
async function finishRun(rep, trigger) {
  ...
  await setCfg({ logs });
  current = null;
  broadcast({ type: "CUU_DONE", log: logs[0] });
}

/* --------------------------- Message hub --------------------------- */
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (!msg) return;
  if (msg.type === "CUU_PROGRESS") { if (current) current.progress = msg.text; broadcast({ type: "CUU_PROGRESS_UI", text: msg.text }); return; }
  if (msg.type === "CUU_REPORT") { finishRun(msg.report, msg.trigger || (current && current.trigger) || "manual").then(() => sendResponse({ ok: true })); return true; }
  if (msg.type === "CUU_RUN_NOW") { startRun("manual").then(sendResponse); return true; }
  if (msg.type === "CUU_STATUS") { chrome.alarms.get(ALARM).then((a) => sendResponse({ current, nextAlarm: a ? a.scheduledTime : null })); return true; }
  if (msg.type === "CUU_TEST_LARK") {
    getCfg().then((cfg) => sendLark(cfg.larkWebhook, `${cfg.larkKeyword} 🐑 Móc Khoá Clicky Chú Cừu 3D – tin nhắn thử từ extension (${fmtTime()}). Kết nối Lark OK.`)).then(sendResponse);
    return true;
  }
  if (msg.type === "CUU_RESCHEDULE") { scheduleAlarm().then(() => sendResponse({ ok: true })); return true; }
  if (msg.type === "CUU_OPEN_OPTIONS") { chrome.runtime.openOptionsPage(); sendResponse({ ok: true }); return; }
});
