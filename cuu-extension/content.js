/* =========================================================================
   ...
   - 1 đơn có thể có NHIỀU item: mỗi item 1 tên. Chỉ có 1 mẫu mockup.
   - Tên khách: External note, dòng thứ n -> item thứ n (theo thứ tự hiển thị).
   - Mockup: ảnh mẫu Chú Cừu + tên khách IN HOA, font Lemonada Bold,
     kiểu sticker: chữ TRẮNG trên blob ĐỎ ôm theo chữ + vòng tròn đỏ bên trái,
     vẽ ở vị trí đo sẵn (góc phải dưới ảnh).
   - Design 1254×1254: nền trắng, chữ đen Arial Bold, canh giữa (giống TKB).
   ========================================================================= */
(() => {
  "use strict";
  if (window.__CUU_LOADED__) return;
  window.__CUU_LOADED__ = true;

  const EXT = (p) => chrome.runtime.getURL(p);
  const clamp = (v, a, b) => Math.max(a, Math.min(b, v));

  /* ======================= Thông số mẫu =======================
     Template = ảnh "chưa điền tên" 1500×1500 (assets/cuu.jpg). Toạ độ đo bằng
     cách khớp ảnh mẫu ĐÃ điền THUYTHU (ví dụ.png) với bản chưa điền:
     ...
     ================================================================== */
  /* Style sticker (giống Cầu lông, tỷ lệ theo cỡ chữ `size`):
  ...
  const TPL = {
    cuu: { key: "cuu", label: "Chú Cừu", img: "assets/cuu.jpg", cx: 966, baseY: 1291, size: 118, maxW: 900 },
  };
  const TPL_ORDER = ["cuu"];
  ...
  /* ------------------------------ Font ------------------------------ */
  const FONT_LEMONADA = "CUU-Lemonada";
  let fontsReady = null;
  function loadFonts() {
  ...
  // Item Chú Cừu: tên sản phẩm "Móc Khoá Clicky Chú Cừu Dễ Thương 3D"
  // (nhận theo chữ "cừu" sau khi bỏ dấu — sửa nếu shop đổi tên/mã)
  const isCuuText = (s) => /\bcuu\b/.test(noAccent(s));

  // Quét các item Chú Cừu trên trang đơn (theo đúng thứ tự hiển thị).
  ...
  function scanItems() {
    ...
    rows.forEach((row, i) => {
      ...
      if (!isCuuText(raw)) return;
      const sku = (raw.match(/SKU ID:\s*([\w.-]+)/i) || [])[1] || "";
      out.push({ rowIndex: i, sku, color: "cuu", detected: true, name: "", sizePct: 100, dx: 0, dy: 0 });
    });
    return out;
  }
  ...
  function buildModal() {
    ...
    overlay.id = "cuu-overlay";
    overlay.innerHTML = `
      <div id="cuu-modal">
        <div class="cuu-head"><h2>🐑 Tạo Mockup &amp; Design – Móc Khoá Clicky Chú Cừu 3D</h2><button class="cuu-close" title="Đóng">×</button></div>
        <div class="cuu-body">
          <div class="cuu-left">
            <div id="cuu-warn" class="cuu-warn" style="display:none"></div>
            <div class="cuu-field"><label>Các item trong đơn (tên lấy theo thứ tự dòng của External note)</label>
              <div id="cuu-items" class="cuu-items"></div>
            </div>
            <div class="cuu-field"><label>Chỉnh chữ cho item đang chọn</label>
              <div class="cuu-rf-row"><span>Cỡ %</span><input type="number" id="cuu-size" class="cuu-num cuu-num-sm" min="40" max="200" step="5" value="100">
                <span>X</span><input type="number" id="cuu-tx" class="cuu-num cuu-num-sm" step="1" value="0">
                <span>Y</span><input type="number" id="cuu-ty" class="cuu-num cuu-num-sm" step="1" value="0">
                <button type="button" id="cuu-treset">Đặt lại</button></div>
              <div class="cuu-hint">Kéo thẳng trên ảnh preview để dời chữ · lăn chuột để đổi cỡ. Tên trên mockup luôn IN HOA, tên quá dài tự thu nhỏ.</div>
            </div>
          </div>
          <div class="cuu-right"><div class="cuu-previews">
            <div class="cuu-pv"><div class="cuu-pv-title" id="cuu-pv-label">Mockup</div>
              <canvas id="cuu-pv-mock" class="cuu-draggable"></canvas></div>
            <div class="cuu-pv"><div class="cuu-pv-title">Design (1254×1254) · nền trắng, chữ đen Arial Bold</div>
              <canvas id="cuu-pv-design"></canvas></div>
          </div></div>
          <div id="cuu-status" class="info"></div>
          <div class="cuu-actions">
            <button class="cuu-btn-dl" id="cuu-dl">⬇ Tải PNG (tất cả)</button>
            <button class="cuu-btn-fill" id="cuu-fill">✅ Điền vào Mockup &amp; Design</button>
          </div>
        </div>
      </div>`;
    ...
    els = {
      overlay, warn: $("#cuu-warn"), list: $("#cuu-items"), size: $("#cuu-size"),
      tx: $("#cuu-tx"), ty: $("#cuu-ty"), tReset: $("#cuu-treset"), pvLabel: $("#cuu-pv-label"),
      pvMock: $("#cuu-pv-mock"), pvDesign: $("#cuu-pv-design"), status: $("#cuu-status"),
      btnDl: $("#cuu-dl"), btnFill: $("#cuu-fill"),
    };
    ...
    $(".cuu-close").onclick = closeModal;
    overlay.addEventListener("click", (e) => { if (e.target === overlay) closeModal(); });
    els.btnDl.onclick = doDownload; els.btnFill.onclick = doFill;

    let drag = null;
    els.pvMock.addEventListener("pointerdown", (e) => { drag = { x: e.clientX, y: e.clientY }; els.pvMock.setPointerCapture(e.pointerId); els.pvMock.classList.add("cuu-grabbing"); });
    ...
    const end = () => { if (drag) { drag = null; els.pvMock.classList.remove("cuu-grabbing"); } };
  ...
  function renderItemList() {
    ...
    state.items.forEach((it, i) => {
      ...
      card.className = "cuu-item" + (i === state.active ? " sel" : "") + (it.name ? "" : " empty");
      card.innerHTML = `
        <div class="cuu-item-head">
          <b>Item ${i + 1}</b>
          <span class="cuu-item-tk ok">Chú Cừu</span>
          <span class="cuu-item-sku">${it.sku || ""}</span>
        </div>
        <div class="cuu-item-row">
          <input class="cuu-item-name" type="text" placeholder="Tên khách…" value="">
        </div>`;
      const input = card.querySelector(".cuu-item-name");
      input.value = it.name || "";
    ...
    if (!state.items.length) els.list.innerHTML = '<div class="cuu-hint">Không thấy item Móc Khoá Chú Cừu nào trên trang này.</div>';
  }
  function select(i) {
    if (i === state.active) { syncTools(); return; }
    state.active = i;
    els.list.querySelectorAll(".cuu-item").forEach((c, k) => c.classList.toggle("sel", k === i));
    syncTools(); schedule();
  }
  function syncTools() {
    ...
    els.pvLabel.textContent = `Mockup – Item ${state.active + 1} · Chú Cừu`;
  }
  function refreshWarn() {
    ...
    const msgs = [];
    if (n && m && m !== n) msgs.push(`⚠️ External note có ${m} dòng tên nhưng đơn có ${n} item Chú Cừu — kiểm tra lại rồi điền tay cho khớp.`);
  ...
  function toast(msg, cls) {
    let t = document.getElementById("cuu-toast");
    if (!t) { t = document.createElement("div"); t.id = "cuu-toast"; document.body.appendChild(t); }
  ...
  async function buildFiles(it, i) {
    ...
    const mock = await canvasToFile(await renderMockup(document.createElement("canvas"), it.color, it.name, { dx: it.dx, dy: it.dy }, it.sizePct / 100), `mockup-cuu-${tag}.png`);
    const des = await canvasToFile(await renderDesign(document.createElement("canvas"), it.name), `design-cuu-${tag}.png`);
    return { mock, des };
  }
  ...
  function waitFillResult(timeout) {
    return new Promise((resolve) => {
      ...
      const on = (ev) => {
        ...
        if (ev.data.__cuu === "progress") { // còn chạy -> gia hạn chờ
        ...
        if (ev.data.__cuu === "result") done(ev.data);
      };
  ...
  async function doFill() {
    try {
      ...
      window.postMessage({ __cuu: "fill", items: payload }, "*");
  ...
  function ensureButton() {
    ...
    let btn = document.getElementById("cuu-open-btn");
    if (!onOrder) { if (btn) btn.remove(); return; }
    if (btn) return;
    btn = document.createElement("button"); btn.id = "cuu-open-btn"; btn.type = "button";
    btn.innerHTML = "🐑 Tạo Mockup Chú Cừu";
    btn.title = "Lấy tên ở External note, dòng thứ n → item Chú Cừu thứ n";
  ...
  window.__CUU_API__ = {
    ...
    noteNames, cleanName, isCuuText, noAccent,
