...
async function refreshNext() {
  const st = await send({ type: "CUU_STATUS" });
  const el = $("#next");
...
$("#runNow").onclick = async () => {
  ...
  const r = await send({ type: "CUU_RUN_NOW" });
...
chrome.runtime.onMessage.addListener((msg) => {
  if (!msg) return;
  if (msg.type === "CUU_PROGRESS_UI") setStatus("Đang chạy: " + msg.text, "");
  if (msg.type === "CUU_DONE") {
    const l = msg.log;
