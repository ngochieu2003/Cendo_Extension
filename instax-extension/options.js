...
const DEFAULTS = {
  ...
  larkWebhook: "", // nhập ở trang cài đặt (repo public — không hardcode webhook)
...
async function refreshNext() {
  const st = await send({ type: "INSTAX_STATUS" });
  const el = $("#next");
...
function renderLogs(logs) {
  ...
  logs.forEach((l) => {
    ...
    d.innerHTML = `<b>${fmt(l.at)}</b> · ${l.trigger === "schedule" ? "theo lịch" : "chạy tay"} · ✅ ${l.done} · ⚠️ ${l.skipped} · ❌ ${l.errors} · đã có ảnh ${l.already}` +
      ...
      `<details><summary style="cursor:pointer;color:#e2711d;font-size:12px">Xem nội dung đã gửi</summary><pre></pre></details>`;
...
$("#runNow").onclick = async () => {
  ...
  const r = await send({ type: "INSTAX_RUN_NOW" });
...
chrome.runtime.onMessage.addListener((msg) => {
  if (!msg) return;
  if (msg.type === "INSTAX_PROGRESS_UI") setStatus("Đang chạy: " + msg.text, "");
  if (msg.type === "INSTAX_DONE") {
    const l = msg.log;
