/* =========================================================================
   Móc Khóa Clicky Instax 3D – script chạy ở MAIN world của cendo.work.

   CƠ CHẾ ĐIỀN: sao chép nguyên cơ chế đã kiểm chứng của tkb-extension v1.1.x
   (gốc là userscript "Cendo → Raccoonie v9.8" chạy ổn trong sản xuất):

   1. KHÔNG giả lập kéo-thả (dragenter/dragover/drop) — báo thành công giả.
   ...
   5. Nút thùng rác: MỌI nút nằm trong [class*="controller_header"] là nút
      ...
      hoặc có chữ "Upload".
   ⚠️ Modal là position:fixed → offsetParent luôn null; "đang mở" phải đo
      bằng getBoundingClientRect().
   ========================================================================= */
(function () {
  "use strict";
  if (window.__INSTAX_MAIN__) return;
  window.__INSTAX_MAIN__ = true;
  ...
  // Nút trong controller_header là nút xoá/thùng rác — không bao giờ bấm
  ...
  function injectFileToInput(input, file) {
    try {
      ...
      console.warn("[INSTAX] injectFileToInput lỗi:", e);
      return false;
    }
  }
  ...
  window.addEventListener("message", async (ev) => {
    if (ev.source !== window || !ev.data || ev.data.__instax !== "fill") return;
    const list = Array.isArray(ev.data.items) ? ev.data.items : [ev.data];
    const results = [];
    const tick = (label, step) => window.postMessage({ __instax: "progress", done: results.length, total: list.length, label, step }, "*");
    ...
    window.postMessage({ __instax: "result", results }, "*");
  });

  /* --------- Chẩn đoán: gõ __INSTAX_DIAG__() trong Console để soi --------- */
  window.__INSTAX_DIAG__ = () => {
    const rows = [...document.querySelectorAll('[class*="media_row"]')];
    return {
      rows: rows.length,
