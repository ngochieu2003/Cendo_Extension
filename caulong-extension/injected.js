/* =========================================================================
   Móc Khoá Cầu Lông 3D – script chạy ở MAIN world của cendo.work.
...
(function () {
  "use strict";
  if (window.__CAULONG_MAIN__) return;
  window.__CAULONG_MAIN__ = true;
  ...
  function injectFileToInput(input, file) {
    try {
      ...
      console.warn("[CAULONG] injectFileToInput lỗi:", e);
      return false;
    }
  }
  ...
  window.addEventListener("message", async (ev) => {
    if (ev.source !== window || !ev.data || ev.data.__caulong !== "fill") return;
    const list = Array.isArray(ev.data.items) ? ev.data.items : [ev.data];
    const results = [];
    const tick = (label, step) => window.postMessage({ __caulong: "progress", done: results.length, total: list.length, label, step }, "*");
    ...
    window.postMessage({ __caulong: "result", results }, "*");
  });

  /* --------- Chẩn đoán: gõ __CAULONG_DIAG__() trong Console để soi --------- */
  window.__CAULONG_DIAG__ = () => {
    const rows = [...document.querySelectorAll('[class*="media_row"]')];
    return {
      rows: rows.length,
