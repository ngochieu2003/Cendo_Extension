...
async function refreshNext() {
  const st = await send({ type: "CAULONG_STATUS" });
  const el = $("#next");
...
function renderLogs(logs) {
  ...
  logs.forEach((l) => {
    ...
    d.innerHTML = `<b>${fmt(l.at)}</b> · ${l.trigger === "schedule" ? "theo lịch" : "chạy tay"} · ✅ ${l.done} · ⚠️ ${l.skipped} · ❌ ${l.errors} · đã có ảnh ${l.already}` +
      ...
      `<details><summary style="cursor:pointer;color:#1240c2;font-size:12px">Xem nội dung đã gửi</summary><pre></pre></details>`;
...
$("#runNow").onclick = async () => {
  ...
  const r = await send({ type: "CAULONG_RUN_NOW" });
...
chrome.runtime.onMessage.addListener((msg) => {
  if (!msg) return;
  if (msg.type === "CAULONG_PROGRESS_UI") setStatus("Đang chạy: " + msg.text, "");
  if (msg.type === "CAULONG_DONE") {
    const l = msg.log;
