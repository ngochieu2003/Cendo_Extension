/* =========================================================================
   Móc Khoá Pickleball 3D – script chạy ở MAIN world của cendo.work.
...
(function () {
  "use strict";
  if (window.__PICKLE_MAIN__) return;
  window.__PICKLE_MAIN__ = true;
  ...
  function injectFileToInput(input, file) {
    try {
      ...
      console.warn("[PICKLE] injectFileToInput lỗi:", e);
      return false;
    }
  }
  ...
  window.addEventListener("message", async (ev) => {
    if (ev.source !== window || !ev.data || ev.data.__pickle !== "fill") return;
    const list = Array.isArray(ev.data.items) ? ev.data.items : [ev.data];
    const results = [];
    const tick = (label, step) => window.postMessage({ __pickle: "progress", done: results.length, total: list.length, label, step }, "*");
    ...
    window.postMessage({ __pickle: "result", results }, "*");
  });

  /* --------- Chẩn đoán: gõ __PICKLE_DIAG__() trong Console để soi --------- */
  window.__PICKLE_DIAG__ = () => {
    const rows = [...document.querySelectorAll('[class*="media_row"]')];
    return {
      rows: rows.length,
