...
async function refreshNext() {
  const st = await send({ type: "PICKLE_STATUS" });
  const el = $("#next");
...
$("#runNow").onclick = async () => {
  ...
  const r = await send({ type: "PICKLE_RUN_NOW" });
...
chrome.runtime.onMessage.addListener((msg) => {
  if (!msg) return;
  if (msg.type === "PICKLE_PROGRESS_UI") setStatus("Đang chạy: " + msg.text, "");
  if (msg.type === "PICKLE_DONE") {
    const l = msg.log;
