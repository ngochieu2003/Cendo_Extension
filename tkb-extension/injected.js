/* =========================================================================
   Thời Khoá Biểu 3D – script chạy ở MAIN world của cendo.work.
...
(function () {
  "use strict";
  if (window.__TKB_MAIN__) return;
  window.__TKB_MAIN__ = true;
  ...
  function injectFileToInput(input, file) {
    try {
      ...
      console.warn("[TKB] injectFileToInput lỗi:", e);
      return false;
    }
  }
  ...
  window.addEventListener("message", async (ev) => {
    if (ev.source !== window || !ev.data || ev.data.__tkb !== "fill") return;
    const list = Array.isArray(ev.data.items) ? ev.data.items : [ev.data];
    const results = [];
    const tick = (label, step) => window.postMessage({ __tkb: "progress", done: results.length, total: list.length, label, step }, "*");
    ...
    window.postMessage({ __tkb: "result", results }, "*");
  });

  /* --------- Chẩn đoán: gõ __TKB_DIAG__() trong Console để soi --------- */
  window.__TKB_DIAG__ = () => {
    const rows = [...document.querySelectorAll('[class*="media_row"]')];
    return {
      rows: rows.length,
