...
async function refreshNext() {
  const st = await send({ type: "TKB_STATUS" });
  const el = $("#next");
...
$("#runNow").onclick = async () => {
  ...
  const r = await send({ type: "TKB_RUN_NOW" });
...
chrome.runtime.onMessage.addListener((msg) => {
  if (!msg) return;
  if (msg.type === "TKB_PROGRESS_UI") setStatus("Đang chạy: " + msg.text, "");
  if (msg.type === "TKB_DONE") {
    const l = msg.log;
